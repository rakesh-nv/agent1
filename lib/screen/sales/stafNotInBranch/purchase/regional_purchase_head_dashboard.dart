...
class RegionalPurchaseHeadDashboard extends StatefulWidget {
  const RegionalPurchaseHeadDashboard({super.key});

  @override
  State<RegionalPurchaseHeadDashboard> createState() => _RegionalPurchaseHeadDashboardState();
}

class _RegionalPurchaseHeadDashboardState extends State<RegionalPurchaseHeadDashboard> {
...
class SubordinatesSalesVsPromiseCard extends StatefulWidget {
  const SubordinatesSalesVsPromiseCard({Key? key}) : super(key: key);

  @override
  State<SubordinatesSalesVsPromiseCard> createState() => _SubordinatesSalesVsPromiseCardState();
}

class _SubordinatesSalesVsPromiseCardState extends State<SubordinatesSalesVsPromiseCard> {
  ...
  Widget build(BuildContext context) {
    return Obx(() {
      ...
      // set default selected subordinate
      selectedSubordinate ??= subordinates.first;
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              /// 🔹 Styled Dropdown like your screenshot
              SizedBox(
                width: double.infinity,
                child: DropdownButtonFormField<Subordinate>(
                  value: selectedSubordinate,
                  isExpanded: true,
                  decoration: InputDecoration(
                    labelText: "Select Subordinate",
                    labelStyle: TextStyle(fontSize: SizeConfig.w(14), color: Colors.black87),
                  ...
                  items: subordinates.map((sub) {
                    return DropdownMenuItem<Subordinate>(
                      value: sub,
                      child: Text(sub.name ?? "N/A", style: TextStyle(fontSize: SizeConfig.w(14))),
                    );
                  }).toList(),
                  onChanged: (value) {
                    setState(() {
                      selectedSubordinate = value;
                    });
                  },
                ),
              ),
              ...
              if (selectedSubordinate != null) _buildSubordinateTile(selectedSubordinate!),
            ],
          ),
        ),
      );
      ...
      children: [
        ...
        ListView.separated(
          ...
          itemBuilder: (context, idx) {
            ...
            Color branchProgressColor = Colors.red;
            if (branchPercentage >= 80) {
              branchProgressColor = Colors.green;
            } else if (branchPercentage >= 50) {
              branchProgressColor = Colors.orange;
            ...
            return Column(
              ...
              children: [
                Text(
                  "${branch.branchAlias}: "
                  "₹${NumberFormat('#,##,###').format(branchActualSales)} "
                  "/ ₹${NumberFormat('#,##,###').format(branchPromise)}",
                  style: TextStyle(fontSize: SizeConfig.w(12)),
                ),
                SizedBox(height: SizeConfig.h(4)),
                LinearProgressIndicator(
                ...
                Align(
                  ...
                  child: Text(
                    "${branchPercentage.toStringAsFixed(1)}%",
                    style: TextStyle(fontSize: SizeConfig.w(10), color: branchProgressColor),
                  ),
                ),
              ],
            );
